...
def user_input():
	...
	while True:
		h = raw_input(OKGREEN("Endpoint (Looks like: xxxxxxxxxxxxxx.iot.us-east-1.amazonaws.com)\n> "))
		if h:
	...
	lpkp  = raw_input(OKGREEN("lambda Private Key Path Default:[%s] Click enter to use defualt\n> " % default))
	if not lpkp:
		lambdaPrivateKeyPath = default
	else:
		lambdaPrivateKeyPath = lpkp
	default = "client.cert.pem"
	clientCertificatePath  = raw_input(OKGREEN("client Certificate Path Default:[%s] Click enter to use defualt\n> " % default))
